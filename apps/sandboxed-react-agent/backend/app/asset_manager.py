...
class AssetManager:
    def __init__(self, asset_repository: AssetRepository | Any) -> None:
        base = os.getenv("ASSET_STORE_PATH", "/app/data/assets")
        self.base_dir = Path(base)
        self.base_dir.mkdir(parents=True, exist_ok=True)
        if isinstance(asset_repository, AssetRepository):
            self.asset_repository = asset_repository
        else:
            self.asset_repository = AssetRepository(asset_repository)
    ...
    def store_base64_asset(
        ...
        *,
        ...
        self.asset_repository.add(record)
        return {
            **record,
    ...
    def get_asset_for_user(self, asset_id: str, user_id: str) -> dict[str, Any] | None:
        return self.asset_repository.get_for_user(asset_id, user_id)

    def get_asset_for_share(
        self, asset_id: str, share_id: str
    ) -> dict[str, Any] | None:
        return self.asset_repository.get_for_share(asset_id, share_id)
